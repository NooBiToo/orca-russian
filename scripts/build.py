...
# защищённые plugin*-пути, которые языковому паку переводить РАЗРЕШЕНО.
...
def protected_translation(path: str) -> bool:
    ...
    if path in TRANSLATABLE_PLUGIN_CHROME:
        return False
    return bool(PROTECTED_MODULE.match(path[len(PROTECTED_ROOT):]))
...
def main() -> int:
    ...
    def walk(node, path="", depth=0):
        ...
        for key, value in node.items():
            ...
            if protected_translation(full) and not (
                isinstance(value, dict) and translatable_plugin_chrome_container(full)
            ):
